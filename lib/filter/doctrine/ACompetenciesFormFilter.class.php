 ...
 * @version    1.0
 */
class ACompetenciesFormFilter extends BaseACompetenciesFormFilter
{
  ...
  public function configure()
  {
	...
	$choice_query = Doctrine::getTable('Competency')
		->createQuery('r')
		->leftJoin('r.Translation t')
		->orderBy('r.competency_id');   

	$this->widgetSchema['competency_id'] = new sfWidgetFormDoctrineChoice(
		array('model' => 'Competency', 'add_empty' => true, 'query' => $choice_query));
	$this->validatorSchema['competency_id'] = new sfValidatorDoctrineChoice(
		array('required' => false, 'model' => 'Competency', 'column' => 'competency_id'));    
    ...
    $query = Doctrine_Core::getTable('ACompetencies')
      ->createQuery('r')
      ->select('a.name, c.competency_id, t.competency, r.description, ac.*')
      ->leftJoin('r.Alumni a')
