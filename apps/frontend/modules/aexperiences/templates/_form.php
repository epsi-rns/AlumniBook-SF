<?php 
	...
	$url_form = url_for('aexperiences/'
		.($isNew ? 'create' : 'update?did='.$did));
?>

<script type="text/javascript">
	window.addEvent("domready",function(){	
		document.id('a_experiences_organization').addClass('required');
		document.id('a_experiences_year_in').addClass('validate-integer');
		document.id('a_experiences_year_out').addClass('validate-integer');
      ...
      <tr>
        <td colspan="2">
          &nbsp;<?php echo link_to(__('Back to list'), 
				'@aexperiences_list?aid='.$aid, $attr_list); ?>
          <?php if (!$isNew): ?>
            &nbsp;<?php echo link_to(__('Delete'), 
				'aexperiences/delete?did='.$did, $attr_dele) ?>
          <?php endif; ?>
