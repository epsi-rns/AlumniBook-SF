<?php 
	...
	$attr_dele = array(
		...
		'class' => 'icon_delete');
	$url_form = url_for('acontacts/'
		.($isNew ? 'create' : 'update?did='.$did));
?>

<script type="text/javascript">
	window.addEvent("domready",function(){	
		document.id('a_contacts_ct_id').addClass('required');
		document.id('a_contacts_contact').addClass('required');
      ...
      <tr>
        <td colspan="2">
          &nbsp;<?php echo link_to(__('Back to list'), 
				'acontacts/list?aid='.$lid, $attr_list); ?>        
          <?php if (!$isNew): ?>
            &nbsp;<?php echo link_to(__('Delete'), 
				'acontacts/delete?did='.$did, $attr_dele) ?>
          <?php endif; ?>
