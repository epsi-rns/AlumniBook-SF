<?php 
	...
	$url_form = url_for('ocontacts/'
		.($isNew ? 'create' : 'update?did='.$did));
?>

<script type="text/javascript">
	window.addEvent("domready",function(){	
		document.id('o_contacts_ct_id').addClass('required');
		document.id('o_contacts_contact').addClass('required');
      ...
      <tr>
        <td colspan="2">
          &nbsp;<?php echo link_to(__('Back to list'), 
				'ocontacts/list?oid='.$lid, $attr_list); ?>
          <?php if (!$isNew): ?>
            &nbsp;<?php echo link_to(__('Delete'), 
				'ocontacts/delete?did='.$did, $attr_dele) ?>
          <?php endif; ?>
