<?php 
	...
	$attr_dele = array(
		...
		'class' => 'icon_delete');
	$url_form = url_for('acompetencies/'
		.($isNew ? 'create' : 'update?did='.$did));
?>

<script type="text/javascript">
	window.addEvent("domready",function(){	
		document.id('a_competencies_competency_id').addClass('required');
      ...
      <tr>
        <td colspan="2">
          &nbsp;<?php echo link_to(__('Back to list'), 
				'@acompetencies_list?aid='.$aid, $attr_list); ?>
          <?php if (!$isNew): ?>
            &nbsp;<?php echo link_to(__('Delete'), 
				'acompetencies/delete?did='.$did, $attr_dele) ?>
          <?php endif; ?>
